...
static void nv_test_klr_tree_ex(int *width, int height)
{
...
static void
nv_test_klr_tree_inherit(nv_klr_tree_t *tree, const nv_klr_tree_t *base)
{
	nv_matrix_t *data = nv_load_matrix(NV_TEST_DATA);
	nv_matrix_t *labels = nv_load_matrix(NV_TEST_LABEL);
	nv_matrix_t *cluster_labels = nv_matrix_alloc(1, data->m);
...
nv_test_klr_tree_nodes(void)
{
	...
	nv_test_klr_tree_ex(width0, 1);
	nv_test_klr_tree_ex(width1, 2);
	nv_test_klr_tree_ex(width2, 3);
	nv_test_klr_tree_ex(width3, 3);
	nv_test_klr_tree_ex(width4, 4);
	nv_test_klr_tree_ex(width5, 2);
	nv_test_klr_tree_ex(width6, 3);
	nv_test_klr_tree_ex(width7, 3);
}

static void
nv_test_klr_tree_inherit_all(void)
{
	nv_matrix_t *data = nv_load_matrix(NV_TEST_DATA);
	int width0[] = { 8, 4 };
	int width1[] = { 8, 2, 2 };
	...
	nv_test_klr_tree_inherit(tree0, NULL);
	nv_test_klr_tree_inherit(tree1, tree0);	
	nv_test_klr_tree_inherit(tree2, NULL);
	nv_test_klr_tree_inherit(tree3, tree2);
...
void
nv_test_klr_tree(void)
{
	nv_test_klr_tree_inherit_all();
	nv_test_klr_tree_nodes();
