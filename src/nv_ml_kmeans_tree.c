...
#define NV_KMEANS_TREE_TOPN 1

static int nv_kmeans_tree_progress_flag = 0;

void
nv_kmeans_tree_progress(int flag)
{
	nv_kmeans_tree_progress_flag = flag;
}

nv_kmeans_tree_t *
nv_kmeans_tree_alloc(int n, const int *dim, int height)
{
	...
	for (y = 0; y < height; ++y) {
		...
		for (x = 0; x < prev_width; ++x) {
			tree->mat[y][x] = nv_matrix_alloc(n, dim[y]);
		}
	...
	tree->n = n;
	tree->m = m;
	if (nv_kmeans_tree_progress_flag) {
		printf("nv_kmeans_tree_alloc: alloc: %d dim\n", tree->m);
		fflush(stdout);
	}
...
int
nv_kmeans_tree_predict_label_ex(const nv_kmeans_tree_t *tree, int height,
	...
	for (y = 0; y < height; ++y) {
		if (y + 2 == height) {
			if (nn <= 1) {
				...
				x = nv_nn(tree->mat[y + 1][idx], vec, vec_j);
				idx *= tree->dim[y + 1];
				idx += x;
			} else {
				...
				for (i = 0; i < nn; ++i) {
					...
					nv_int_float_t ld = nv_nn_dist(tree->mat[y + 1][tmp_idx],
												  vec, vec_j);
					if (ld.f < min_dist) {
...
nv_int_float_t
nv_kmeans_tree_predict_label_and_dist_ex(const nv_kmeans_tree_t *tree, int height,
										 const nv_matrix_t *vec, int vec_j, int nn)
{
	int y, x, idx = 0;
	nv_int_float_t ld;
...
void
nv_kmeans_tree_train(nv_kmeans_tree_t *tree,
...
#pragma omp parallel for
		...
		if (nv_kmeans_tree_progress_flag) {
			printf("nv_kmeans_tree_train: vq: %ldms\n", nv_clock() - t);
			fflush(stdout);
		}
		
		for (x = 0; x < prev_width; ++x) {
			if (y == 0) {
				nv_kmeans_tree_train_at(tree, data, y, x, max_epoch);
			} else {
				int i, j = 0;
				...
				if (j != 0) {
					...
					nv_kmeans_tree_train_at(tree, data_part, y, x, max_epoch);
					nv_matrix_free(&data_part);
				} else {
					if (nv_kmeans_tree_progress_flag) {
						printf("nv_kmeans_tree_train: no data: y:%d, x:%d\n",
							   y, x);
						fflush(stdout);
					}
				}
...
void
nv_kmeans_tree_train_at(nv_kmeans_tree_t *tree,
						const nv_matrix_t *data, int y, int x, int max_epoch)
{
	...
	nv_kmeans(mat, count, labels, data, mat->m, max_epoch);
	if (nv_kmeans_tree_progress_flag) {
		printf("nv_kmeans_tree_train_at: (%d, %d), data: %d, %ldms\n",
...
void
nv_kmeans_tree_inherit_train(nv_kmeans_tree_t *tree,
	...
	for (y = 0; y < tree->height; ++y) {
		...
		if (inherit) {
				...
				base_tree->dim[y] == tree->dim[y])
			{
				...
				if (nv_kmeans_tree_progress_flag) {
					printf("nv_kmeans_tree_train: inherit: (%d, 0-%d)\n",
						   y, prev_width);
					fflush(stdout);
				}
				prev_width = tree->dim[y] * prev_width;
...
int
nv_kmeans_tree_predict_label(const nv_kmeans_tree_t *tree, const nv_matrix_t *vec, int vec_j)
{
	return nv_kmeans_tree_predict_label_ex(tree, tree->height, vec, vec_j, NV_KMEANS_TREE_TOPN);
}

nv_int_float_t
nv_kmeans_tree_predict_label_and_dist(const nv_kmeans_tree_t *tree,
...
void
nv_kmeans_tree_free(nv_kmeans_tree_t **tree)
{
	if (tree && *tree) {
		int y, x;
		for (y = 0; y < (*tree)->height; ++y) {
